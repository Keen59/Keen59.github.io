  ...
  return (
    <aside className="fixed inset-y-0 left-0 z-30 flex w-56 flex-col bg-harbor text-white">
      <div className="relative overflow-hidden border-b border-white/10 px-5 py-5">
        <div className="pointer-events-none absolute -right-6 -top-6 h-24 w-24 rounded-full bg-brand/40 blur-2xl" />
        <Link to="/" className="relative flex items-center gap-2.5">
          <span className="flex h-9 w-9 items-center justify-center rounded-xl bg-brand text-brand-ink shadow-lift">
            <Anchor className="h-4 w-4" />
          </span>
          <div>
            <p className="text-lg font-bold tracking-tight text-white">
              ESC<span className="text-brand">Argo</span>
            </p>
      ...
      <div className="px-3 pt-4">
        <NavLink
          to="/orders"
          className="flex items-center justify-center gap-2 rounded-xl bg-brand px-3 py-2.5 text-sm font-semibold text-brand-ink transition hover:bg-brand-light"
        >
      ...
      <nav className="mt-3 flex flex-1 flex-col gap-0.5 overflow-y-auto px-3 pb-2">
          ...
          <NavLink
            ...
            className={({ isActive }) =>
              `flex items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium transition ${
                isActive
                  ? 'bg-white/10 text-white shadow-[inset_3px_0_0_0_#E4BFB3]'
